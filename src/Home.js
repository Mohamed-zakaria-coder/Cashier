...
const Home = (props) => {
  ...
  function filterProducts(id) {
    if (search.length > 1) {
      setProducts(
        Data.filter((prod) => prod.name.toLowerCase().includes(search)).map(
          (item) => {
            return (
              <Pizza
                key={item.id}
  ...
  const [selectedQuantity, setSelected] = React.useState(0);
  ...
  const [selectedPizza, setSelectedPizza] = React.useState(Data);
  ...
  const category = categories.map((category) => {
    return (
      ...
      >
        ...
        <div>{`${category.name[0].toUpperCase()}${category.name.slice(
          1
        )}`}</div>
      </div>
    );
    ...
    <div className="container grow">
      <div className="grow">
        <div className="menu-category-parent">
          <h2 className="menu-category-heading">Menu Category</h2>
        ...
        <div className="categories-parent">{category}</div>
        <div className="choose-sort-parent">
          <h2 className="choose-order">Choose Order</h2>
        </div>
        <div className="pizza-container">{products.length > 0 && products}</div>
      </div>

      <div
        className="out-line-parent"
